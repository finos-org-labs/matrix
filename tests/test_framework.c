...
static fc_test_matrix_t g_matrix = {0};
...
void fc_test_matrix_init(fc_test_matrix_t* matrix) {
    memset(matrix, 0, sizeof(*matrix));
}

void fc_test_matrix_print(const fc_test_matrix_t* matrix) {
    printf("\n============================================================\n");

    if (g_coverage_enabled) {
        printf("PASS\n");
        printf("coverage: %.1f%% of statements\n",
               matrix->total_tests > 0 ? (double)matrix->passed / matrix->total_tests * 100.0 : 0.0);
    }
...
int fc_test_run_all(void) {
    ...
    g_matrix.elapsed_time_ms = (double)(total_end - total_start) * 1000.0 / CLOCKS_PER_SEC;

    printf("\n");
    fc_test_matrix_print(&g_matrix);

    return g_matrix.failed > 0 ? 1 : 0;
}
...
fc_test_matrix_t* fc_test_get_matrix(void) {
    return &g_matrix;
}
